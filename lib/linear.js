...
export function buildIssueBody(teamKey, number) {
  ...
  const fields = 'identifier title branchName description url priorityLabel estimate state { name } assignee { displayName } team { key } labels { nodes { name } } project { name } cycle { number name } comments(first: 100) { nodes { id createdAt body user { displayName } externalUser { name } parent { id } } }';
