...
export function eventWorktree(env) {
  ...
  const find=(node,depth=0)=>{
    ...
    if(wt && (wt.checkout_path||wt.path))return {path:wt.checkout_path||wt.path,workspace:node.workspace_id||node.workspace?.workspace_id||context.workspace_id};
...
export async function restore({env=process.env,call=client(env.HERDR_BIN_PATH||'herdr'),prepareFn=prepare,askFn=ask}={}) {
  ...
  try {
    ...
    const panes=call(['pane','list','--workspace',ws.workspace_id]).panes;
    const root=panes.find(p=>p.tab_id===ws.active_tab_id && p.pane_id!==env.HERDR_PANE_ID) || panes[0];
    if(!root)throw new Error('The workspace has no panes.');
    ...
    ensureLayout({result:{workspace:ws,tab:{tab_id:ws.active_tab_id},root_pane:root,already_open:true},identifier:ticket.identifier,mode:ticket.mode,cwd:found.path,call,onProgress:console.log});
  }finally{release();}
}
